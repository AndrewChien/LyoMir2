            ...
            try
            {
                ...
                switch (msgPacket.Ident)
                {
                    case Grobal2.GM_OPEN:
                        LogService.Info($"M2Server：执行网关封包消息{Grobal2.GM_OPEN}");
                    ...
                    case Grobal2.GM_CLOSE:
                        LogService.Info($"M2Server：执行网关封包消息{Grobal2.GM_CLOSE}");
                        CloseUser(msgPacket.Socket);
                        break;
                    case Grobal2.GM_CHECKCLIENT:
                        LogService.Info($"M2Server：收到来自GameGate游戏网关心跳包...");
                        GateInfo.SendKeepAlive = true;
                        break;
                    case Grobal2.GM_RECEIVE_OK:
                        LogService.Info($"M2Server：执行网关封包消息{Grobal2.GM_RECEIVE_OK}");
                        GateInfo.SendChecked = 0;
                        GateInfo.SendBlockCount = 0;
                        break;
                    case Grobal2.GM_DATA:
                        LogService.Info($"M2Server：执行网关封包消息{Grobal2.GM_DATA}");
        ...
        private bool GetCertification(ReadOnlySpan<char> sMsg, ref string sAccount, ref string sChrName, ref int nSessionId, ref int nVersion, ref bool boFlag, ref byte[] tHwid, ref int gateId)
        {
            ...
            try
            {
                ...
                if (sData.Length > 2 && sData[0] == '*' && sData[1] == '*')
                {
                    ...
                    LogService.Info($"Account:[{sAccount}] ChrName:[{sChrName}] Code:[{sCodeStr}] ClientVersion:[{sClientVersion}] HWID:[{sHwid}]");
                }
            }
            catch (Exception ex)
            {
