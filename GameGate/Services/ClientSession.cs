...
namespace GameGate.Services
{
    ...
    public class ClientSession : IDisposable
    {
        ...
        public unsafe void ProcessSessionPacket(ClientPacketMessage messagePacket)
        {
            ...
            //LogService.Info($"GameGate：收到来自客户端的消息");
            ...
            if (HandleLogin)
            {
                ...
                if (deCodeLen < 12)
                {
                    LogService.Info("解析数据包小于12...解析后长度:" + deCodeLen);
                    _session.Socket.Close();//关闭异常会话
                    return;
                }
                ...
                LogService.Info($"GameGate：收到消息[Messages.{clientMessage.Ident}]");
        ...
        private void SendDelayMsg(int magicId, byte nDir, int nIdx, int nLen, byte[] pMsg, int delayTime)
        {
            ...
            if (magicId > 0)
            {
                LogService.Info($"发送延时处理消息:User:[{_session.ChrName}] MagicID:[{magicId}] DelayTime:[{delayTime}]");
            }
        }
        ...
        public async Task ProcessServerPacket(byte serviceId, ServerSessionMessage message)
        {
            ...
            //LogService.Info($"GameGate：收到来自GameSrv（5000角色控制处理结果）的消息");
            ...
            else
            {
                ...
                if (bufferLen > 8)
                {
                    ...
                    LogService.Info($"GameGate：收到消息[Messages.{commandMessage.Ident}]");
                    switch (commandMessage.Ident)
                    {
                        ...
                        case Messages.SM_HWID:
                            if (Config.IsProcClientHardwareID)
                            {
                                ...
                                {
                                    case 1:
                                        LogService.Info("封机器码");
                                        break;
                                    case 2:
                                        LogService.Info("清理机器码");
        ...
        private void ClientLogin(string loginData, int nLen, string addr, ref bool success)
        {
            ...
            if (nLen < firstPakcetMaxLen && nLen > 15)
            {
                ...
                if ((sAccount.Length >= 4) && (sAccount.Length <= 12) && (sHumName.Length > 2) && (sHumName.Length < 15))
                {
                    ...
                    //LogService.Info($"[ClientLogin] {sAccount} {sHumName} {addr} {szCert} {szClientVerNo} {szCode} {MD5.MD5Print(hardWareDigest)} {ServiceId}");
                    success = true;
                    HandleLogin = true;
