...
{
    /// <summary>
    /// 面向DBSVr、GameSvr的数据服务（开启LoginSrv：5600）
    /// </summary>
    public class SessionServer
    {
        private readonly IList<ServerSessionInfo> _serverList = null;
        /// <summary>
        /// to（client）：DBSVr、GameSvr
        /// self（server）：5600，数据服务
        /// </summary>
        ...
        private Task Connecting(object sender, TouchSocketEventArgs e)
        {
            ...
            if (boAllowed)
            {
                ...
                LogService.Info($"{client.MainSocket.RemoteEndPoint}链接成功.");
            }
            else
            {
            ...
            for (int i = 0; i < _serverList.Count; i++)
            {
                ...
                if (msgServer.SocketId == socketId)
                {
                    ...
                    while (sReviceMsg.IndexOf(")", StringComparison.OrdinalIgnoreCase) > 0)
                    {
                        ...
                        //记录非心跳日志
                        //if(nCode!= Messages.SS_SERVERINFO)
                        //{
                        //    LogService.Info($"LoginGate（5600）：收到来自DBSVr或GameSvr{msgServer.EndPoint.Address}:{msgServer.EndPoint.Port}消息类型{nCode}");
                        //}

                        switch (nCode)
                        {
                            case Messages.SS_SOFTOUTSESSION:
                                LogService.Info($"LoginSrv：收到[Messages.SS_SOFTOUTSESSION]类型消息");
                                sMsg = HUtil32.GetValidStr3(sMsg, ref sAccount, '/');
                                CloseUser(msgServer, sAccount, HUtil32.StrToInt(sMsg, 0));
                                break;
                            case Messages.SS_SERVERINFO:   //DBServer发心跳消息：103/热血传奇/99/0
                                //LogService.Info($"LoginSrv：收到[Messages.SS_SERVERINFO]类型消息");
                            ...
                            case Messages.ISM_GAMETIMEOFTIMECARDUSER:
                                LogService.Info($"LoginSrv：收到[Messages.ISM_GAMETIMEOFTIMECARDUSER]类型消息");
                                sMsg = HUtil32.GetValidStr3(sMsg, ref sServerName, '/');
                                sMsg = HUtil32.GetValidStr3(sMsg, ref sAccount, '/');
                                ChanggePlayTimeUser(msgServer, sServerName, sAccount, HUtil32.StrToInt(sMsg, 0));
                                break;
                            case Messages.ISM_QUERYACCOUNTEXPIRETIME:
                                LogService.Info($"LoginSrv：收到[Messages.ISM_QUERYACCOUNTEXPIRETIME]类型消息");
                                sMsg = HUtil32.GetValidStr3(sMsg, ref sAccount, '/');
                                QueryPlayTime(msgServer, sAccount);
                                break;
                            case Messages.ISM_CHECKTIMEACCOUNT:
                                LogService.Info($"LoginSrv：收到[Messages.ISM_CHECKTIMEACCOUNT]类型消息");
                                sMsg = HUtil32.GetValidStr3(sMsg, ref sAccount, '/');
                                CheckTimeAccount(msgServer, sAccount);
                                break;
                            case Messages.UNKNOWMSG:
                                LogService.Info($"LoginSrv：收到[Messages.UNKNOWMSG]类型消息");
                                SendServerMsgA(Messages.UNKNOWMSG, sMsg);
                                break;
                            default:
                                LogService.Info($"LoginSrv：收到[{nCode}]类型消息");
                ...
                if (string.Compare(LsShare.CertList[i].LoginID, account, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    if (!LsShare.CertList[i].FreeMode && !LsShare.CertList[i].Closing)
                    {
                        if ((certUser.AvailableType == 2) || ((certUser.AvailableType >= 6) && (certUser.AvailableType <= 10)))
                        {
                            ...
                            LogService.Info($"[GameServer/Send] ISM_QUERYPLAYTIME : {certUser.LoginID} PlayTime: ({seconds})");
                        }
                    }
                }
            }
        ...
        private void ChanggePlayTimeUser(ServerSessionInfo serverInfo, string serverName, string account, int gameTime)
        {
            ...
            if (seconds > 0)
            {
                ...
                LogService.Info($"账号:[{account}] 数据库时间:{seconds} 引擎时间:[{gameTime}]");
                if (seconds < gameTime)
                {
                    LogService.Info($"账号[{account}]游戏时间异常.");
                }
                else
                {
                    seconds = 0;
            ...
            else
            {
                ...
                LogService.Info($"[GameServer/Send] ISM_QUERYPLAYTIME : {account} PlayTime: ({seconds})");
            }
        }
        ...
        private void SendCancelAdmissionUser(string serverName, CertUser certUser)
        {
            ...
            LogService.Info($"[GameServer/Send] ISM_CANCELADMISSION : {certUser.LoginID} TO ({certUser.Addr})");
        }
        ...
        public void SendServerMsg(short wIdent, string sServerName, string sMsg)
        {
            ...
            try
            {
                string tempName = GetLimitName(sServerName);//!UserLimit.txt
                string sSendMsg = string.Format(sFormatMsg, wIdent, sMsg);//(100/qtanchun/2/0/0/127.0.0.1/0)
                for (int i = 0; i < _serverList.Count; i++)
                {
                    ServerSessionInfo msgServer = _serverList[i];
                    if (msgServer.Socket.Connected)
                    {
                        if ((string.IsNullOrEmpty(tempName)) 
                            || (string.IsNullOrEmpty(msgServer.ServerName)) 
                            || (string.Compare(msgServer.ServerName, tempName, StringComparison.OrdinalIgnoreCase) == 0)   //字符串相等
                            || (msgServer.ServerIndex == 99))
                        {
                            msgServer.Socket.Send(HUtil32.GetBytes(sSendMsg));//向连接到LoginSrv（5600）的所有客户端发送消息
                            LogService.Info($"LoginSrv(5600)：向客户端{msgServer.ServerName}:{msgServer.Socket.RemoteEndPoint.GetIP()}:{msgServer.Socket.RemoteEndPoint.GetPort()}发送消息:{sSendMsg}");
                        }
                    }
                }
            }
