...
{
    /// <summary>
    /// 面向LoginGate的登录服务（开启LoginSrv：5500）
    /// </summary>
    public class LoginServer
    {
        ...
        {
            if (e.ByteBlock!= null) //e.ByteBlock为空是心跳包
            {
                LogService.Info($"LoginSrv：收到来自{socketClient?.IP}:{socketClient?.Port}的消息");
            }
            else
            {
                LogService.Info($"LoginSrv：收到来自LoginGate登录网关心跳包...");
            }
        ...
        private void ReceiveCloseUser(string sSockIndex, LoginGateInfo gateInfo)
        {
            ...
            for (int i = 0; i < gateInfo.UserList.Count; i++)
            {
                ...
                {
                    LogService.Info(string.Format(sCloseMsg, userInfo.UserIPaddr));
        ...
        private void ReceiveOpenUser(ServerDataMessage dataMessage, LoginGateInfo gateInfo)
        {
            ...
            LogService.Info(string.Format(sOpenMsg, sUserIPaddr, sGateIPaddr));
        }
