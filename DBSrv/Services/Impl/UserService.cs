...
namespace DBSrv.Services.Impl
{
    ...
    public class UserService : IService
    {
        ...
        /// <summary>
        /// to(client)：角色网关SelGate
        /// self(server)：角色数据服务（5100）
        ...
        private Task Received(ITcpClientBase socketClient, ReceivedDataEventArgs e)
        {
            ...
            //LogService.Info($"DBSrv(5100)：收到来自{(socketClient as SocketClient)?.IP}:{(socketClient as SocketClient)?.Port}的消息");
        ...
        [DynamicDependency(DynamicallyAccessedMemberTypes.PublicProperties, typeof(ServerDataMessage))]
        private void ProcessGateData(ServerDataPacket packetHead, byte[] data, int connectionId, ref SelGateInfo gateInfo)
        {
            try
            {
                ...
                {
                    LogService.Info($"解析角色网关封包出现异常...");
                    return;
                }
                ServerDataMessage messageData = SerializerUtil.Deserialize<ServerDataMessage>(data);
                //Console.WriteLine($"Debug解析包:{messageData.Type}|{messageData.SocketId}|{messageData.DataLen}|{messageData.Data}");

                switch (messageData.Type)
                {
                    case ServerDataType.KeepAlive:
                        //LogService.Info($"DBSrv：收到[ServerDataType.KeepAlive]类型消息");
                        SendKeepAlivePacket(gateInfo.ConnectionId);
                        break;
                    case ServerDataType.Enter:
                        LogService.Info($"DBSrv：收到[ServerDataType.Enter]类型消息");
                        string sData = string.Empty;
                        string sTemp = string.Empty;
                    ...
                    case ServerDataType.Leave:
                        LogService.Info($"DBSrv：收到[ServerDataType.Leave]类型消息");
                        CloseUser(messageData.SocketId, ref gateInfo);
                        break;
                    case ServerDataType.Data:
                        LogService.Info($"DBSrv：收到[ServerDataType.Data]类型消息");
        ...
        private void SendKeepAlivePacket(string connectionId)
        {
            ...
            //Console.WriteLine($"Debug生成包:{message.Type}|{message.SocketId}|{message.DataLen}|{message.Data}");
                ...
                case Messages.CM_QUERYCHR:
                    if (!userInfo.boChrQueryed || ((HUtil32.GetTickCount() - userInfo.dwChrTick) > 200))
                    {
                        ...
                        if (QueryChr(sText, ref userInfo, ref gateInfo))
                        {
                            ...
                            LogService.Info("[QueryChr]:" + sText);
                        }
                        else
                        {
                            LogService.Info("[QueryChr]:" + sText);
                    ...
                    break;
                case Messages.CM_NEWCHR:    //lyo：建角色测试成功
                    ...
                    for (int i = 0; i < chrList.Count; i++)
                    {
                        ...
                        if (result && !humRecord.Deleted)
                        {
                            ...
                            if (_playDataStorage.GetQryChar(nIndex, ref chrRecord))
                            {
                                ...
                                //角色查询，返回给客户端：*qqqqqqq/0/1/0/0/zzzzz/2/1/0/1/
                                sSendMsg += sChrName + "/" + chrRecord.Job + "/" + chrRecord.Hair + "/" + chrRecord.Level + "/" + chrRecord.Sex + "/";
                                nChrCount++;
                            }
                        }
                    }
        ...
        private bool SelectChr(string sData, SelGateInfo curGate, ref SessionUserInfo userInfo)
        {
            ...
            if (boDataOk)
            {
                ...
                SendUserSocket(curGate.ConnectionId, userInfo.SessionId, sDefMsg + sRouteMsg);//lyo：选角色消息返回给SelGate
                _loginService.SetGlobaSessionPlay(userInfo.nSessionID);//登录信息保存在DBServer全局缓存
                result = true;
                LogService.Info($"玩家获取游戏网关信息 RunGame:{sRouteIp} Port:{nRoutePort + nMapIndex}");
            }
            else
            {
