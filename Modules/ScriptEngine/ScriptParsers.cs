...
namespace ScriptSystem
{
    ...
    public class ScriptParsers : IScriptParsers
    {
        ...
        //private readonly Dictionary<string, string> CallScriptDict = new Dictionary<string, string>();//lyo，改为路径+名称查找
        /// <summary>
        /// 所有子脚本中CALL脚本列表
        /// </summary>
        private readonly List<string> CallScriptDict = new List<string>();
        ...
        /// <summary>
        /// 验证脚本中[sLabel]标签的脚本内容{}完整性，并返回脚本内容
        /// </summary>
            ...
            if (File.Exists(sFileName))
            {
                ...
                for (int i = 0; i < callStrList.Count; i++)
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(sLine) && (sLine.Contains(sLabel) || findLab))
                        {
                            if (!findLab)
                            {
                                if (sLine[0] == '[' && string.Compare(sLine, sLabel, StringComparison.OrdinalIgnoreCase) == 0)//找到[sLabel]标签
                                {
                                    findLab = true;
                                    continue;
                                }
                            }
                            if (sLine[0] != '{')//[sLabel]标签下面的内容验证{}的完整性
                            {
                                if (sLine[0] == '}')
                                {
                                    result = true;
        ...
        /// <summary>
        /// 从根（QFunction-0.txt）到叶（所以自定义脚本）加载所有脚本，将其存入一个大的“完整function”中
        /// 注意：所有脚本中，自定义函数名称不能重复
        /// </summary>
        /// <param name="LoadList">“完整function”内容</param>
        /// <param name="success">是否已完成对所有脚本内容的处理</param>
        private void LoadCallScript(ref StringList LoadList, ref bool success)
        {
            ...
            for (int i = 0; i < LoadList.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(sLine) && sLine[0] == '#' && HUtil32.CompareLStr(sLine, "#CALL"))//#CALL开头，执行函数体替换
                {
                    ...
                    try
                    {
                        //1、非首次执行函数，将#CALL转换为#ACT goto存入待执行队列
                        if (CallScriptDict.Contains($"{sFileName}|{sLabName}"))
                        {
                            callCount--;
                            //callList[i] = "#ACT";
                            //callList.InsertText(i + 1, "goto " + sLabName);
                            callList.AppendText($"#ACT");
                            callList.AppendText($"goto {sLabName}");
                            break;
                        }

                        //2、首次执行函数，验证标签函数代码段完整性，并将该标签函数代码全段存入全function
                        callList.AppendText($"goto {sLabName}");//CALL替换成goto函数名标记
                        callList.AppendText($"[{sLabName}]");//存入函数名标记
                ...
                {
                    callList.AppendText(sLine);//非#CALL开头的存入待执行队列
                }
            }
            LoadList = callList;//将CALL的子脚本内容放入预加载脚本容器，递归执行加载
            success = callCount <= 0;//预加载脚本容器消耗完返回成功
        }
        ...
        public void LoadScriptFile(INormNpc NPC, string sPatch, string sScritpName, bool boFlag)
        {
            ...
            if (File.Exists(sScritpFileName))
            {
                ...
                while (!success)
                {
                    LoadCallScript(ref stringList, ref success);//循环加载脚本，直到返回成功
                }
                            ...
                            case 11:
                                {
                                    ...
                                    {
                                        LogService.Error($"脚本错误: {line}第:{i+1}行: {sScritpFileName}");
