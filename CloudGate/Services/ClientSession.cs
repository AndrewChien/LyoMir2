    ...
    public class ClientSession
    {
        ...
        public void HandleSessionPacket(TMessageData message)
        {
            ...
            switch (_handleLogin)
            {
                ...
                case >= 2:
                    {
                        if (message.BufferLen < ClientMesaagePacket.PackSize) // 小于数据包大小的为无效封包
                        {
                            ...
                            case Grobal2.CM_SAY:
                                ...
                                if (nDeCodeLen > ClientMesaagePacket.PackSize)
                                {
                                    if (sMsg.StartsWith("@"))
                                    {
                                        ...
                                        Buffer.BlockCopy(message.Buffer, ClientMesaagePacket.PackSize, pszChatBuffer, 0, nDeCodeLen - ClientMesaagePacket.PackSize);
                        ...
                        if (nDeCodeLen > ClientMesaagePacket.PackSize)
                        {
                            ...
                            Buffer.BlockCopy(packBuff, 0, BodyBuffer, GameServerPacket.PacketSize, ClientMesaagePacket.PackSize);
                            Buffer.BlockCopy(tempBuff, 16, BodyBuffer, 32, tLen); //消息体
                        }
                        else
                        {
                            BodyBuffer = new byte[GameServerPacket.PacketSize + packBuff.Length];
                            cmdPack.PackLength = ClientMesaagePacket.PackSize;
                            Buffer.BlockCopy(packBuff, 0, BodyBuffer, GameServerPacket.PacketSize, packBuff.Length);
                        }
        ...
        private void SendFirstPack(byte[] packet, int len = 0)
        {
            ...
            GateMsg.PackLength = tempBuff.Length - GameServerPacket.PacketSize;//只需要发送数据封包大小即可
        ...
        private void SendSysMsg(string szMsg)
        {
            ...
            Cmd.Direct = 0;
            SendBuf[0] = (byte)'#';
            Buffer.BlockCopy(Cmd.GetBuffer(), 0, TempBuf, 0, ClientMesaagePacket.PackSize);
