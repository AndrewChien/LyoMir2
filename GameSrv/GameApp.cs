...
namespace GameSrv
{
    /// <summary>
    /// 主引擎，单例模式，初始化M2属性变量库
    /// （基类中开启GameSvr:5000，并开启客户端连接LoginSvr:5600、DBSvr:6000）
    ...
    public class GameApp : ServerBase
    {
        ...
        private static void InitializeSystem(IServiceProvider serviceProvider)
        {
            ...
            SystemShare.Mediator = serviceProvider.GetService<IMediator>();//在MediatR中介者中注册
        }
        ...
        public void Initialize(CancellationToken stoppingToken)
        {
            ...
            nCode = Map.LoadMapInfo(GameSrvPub.OutputMode);
            if (nCode < 0)
            {
            ...
            M2Share.NetChannel.Initialize();//初始化游戏网关（5000端口）
