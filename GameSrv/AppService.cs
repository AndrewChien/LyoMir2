...
namespace GameSrv
{
    ...
    public class AppService : IHostedLifecycleService, IDisposable
    {
        ...
        /// <returns></returns>
        public static AppService BuildAppService(IServiceProvider serverApp)
        {
            return new AppService(new GameApp(serverApp));
        }
        ...
        public AppService(GameApp serverApp)
        {
            ...
            LogService.Info($"Starting with arguments: {string.Join(" ", Environment.GetCommandLineArgs())}");
        ...
        {
            _exitCode = 0;
            await _mirApp.StartUp(cancellationToken);//（开启GameSvr:5000，并开启客户端连接LoginSvr:5600、DBSvr:6000）
        ...
        {
            LogService.Info($"Exiting with return code: {_exitCode}");
            return Task.CompletedTask;
        }
