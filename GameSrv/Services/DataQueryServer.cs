...
{
    /// <summary>
    /// 玩家数据读写服务（GameSvr客户端 --> DBSvr:6000）
    /// </summary>
    public class DataQueryServer
    {
        ...
        /// <summary>
        /// 初始化玩家数据读写服务（GameSvr客户端 --> DBSvr:6000）
        /// </summary>
        public void Initialize()
        {
            _tcpClient.Setup(new TouchSocketConfig()
            .SetRemoteIPHost(new IPHost(IPAddress.Parse(SystemShare.Config.sDBAddr), SystemShare.Config.nDBPort))   //6000端口
        ...
        /// <summary>
        /// 启动玩家数据读写服务（GameSvr客户端 --> DBSvr:6000）
        /// </summary>
        /// <returns></returns>
        ...
        {
            LogService.Info($"GameSrv：收到来自{sender?.IP}:{sender?.Port}的玩家数据读写服务消息");
