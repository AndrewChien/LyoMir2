...
{
    /// <summary>
    /// 账号会话认证服务（GameSvr客户端 --> LoginSvr:5600）
    /// </summary>
    public class AuthenticationService : IAuthentication
    {
        ...
        private Task DataSocketRead(TcpClient sender, ReceivedDataEventArgs e)
        {
            ...
            try
            {
                ...
                _socketRecvText += msg;
                //心跳连接信息
                //LogService.Info($"GameSrv：收到来自{sender?.IP}:{sender?.Port}的账号会话认证服务消息");
            }
            finally
            {
                ...
                while (true)
                {
                    ...
                    switch (HUtil32.StrToInt(sCode, 0))
                    {
                        case Messages.SS_OPENSESSION:// 100
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.SS_OPENSESSION]类型消息");
                            GetPasswdSuccess(sBody);
                            break;
                        case Messages.SS_CLOSESESSION:// 101
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.SS_CLOSESESSION]类型消息");
                            GetCancelAdmission(sBody);
                            break;
                        case Messages.SS_KEEPALIVE:// 104
                            //LogService.Info($"GameSrv账号验证服务：收到[Messages.SS_KEEPALIVE]类型消息");
                            SetTotalHumanCount(sBody);
                            break;
                        case Messages.UNKNOWMSG:
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.UNKNOWMSG]类型消息");
                            break;
                        case Messages.SS_KICKUSER:// 111
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.SS_KICKUSER]类型消息");
                            GetCancelAdmissionA(sBody);
                            break;
                        case Messages.SS_SERVERLOAD:// 113
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.SS_SERVERLOAD]类型消息");
                            GetServerLoad(sBody);
                            break;
                        case Messages.ISM_ACCOUNTEXPIRED:
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.ISM_ACCOUNTEXPIRED]类型消息");
                            GetAccountExpired(sBody);
                            break;
                        case Messages.ISM_QUERYPLAYTIME:
                            LogService.Info($"GameSrv账号验证服务：收到[Messages.ISM_QUERYPLAYTIME]类型消息");
